          ...
          >
            ...
            <div className="bg-gradient-to-r from-primary to-primary-dark p-6 rounded-t-2xl">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center text-white font-black text-xl flex-shrink-0`}>
                    {expanded.creatorId?.fullName?.[0] || 'C'}
                  </div>
                  <div>
                    <h2 className="text-lg font-black text-white">{expanded.creatorId?.fullName}</h2>
                    <p className="text-purple-200 text-sm">{expanded.creatorId?.email}</p>
                    {expanded.creatorId?.socialPlatform && (
                      <p className="text-purple-300 text-xs mt-0.5">{expanded.creatorId.socialPlatform}</p>
                    )}
                  </div>
                </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="bg-primary-light rounded-xl p-4">
                ...
                <p className="font-bold text-secondary">{expanded.opportunityId?.title}</p>
                <p className="text-xs text-muted mt-1">
                  {expanded.opportunityId?.platform} · {expanded.opportunityId?.category}
                </p>
              </div>
              ...
              {expanded.note && (
                <div>
                  <h3 className="text-sm font-bold text-secondary mb-2">📝 Creator's Proposal</h3>
              ...
              {expanded.status === 'countered' && (
                <div>
                  <h3 className="text-sm font-bold text-secondary mb-2">🔄 Counter Offer</h3>
                  <div className={`rounded-xl p-4 ${
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <p className={`text-xs font-bold ${
                        expanded.lastCounterBy === 'creator' ? 'text-primary' : 'text-blue-700'
                      }`}>
                        {expanded.lastCounterBy === 'creator'
    ...
    >
      {expanded.creatorId.socialProfileUrl}
              ...
              <p className="text-xs text-muted">
                Applied on: {new Date(expanded.createdAt).toLocaleDateString('en-PK', {
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-purple">
            ...
            <p className="text-sm text-muted mb-1">
              To: <span className="font-semibold text-secondary">{counterModal.creatorId?.fullName}</span>
            </p>
            <p className="text-sm text-muted mb-5">
              Creator offered: <span className="font-bold text-primary">
          ...
          {applications.map((app, i) => (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap mb-2">
                    <h3 className="font-bold text-secondary text-sm">{app.opportunityId?.title}</h3>
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${statusColors[app.status]}`}>
                      {statusLabels[app.status]}
                    </span>
                  </div>
                  ...
                  {app.status === 'countered' && app.lastCounterBy === 'creator' && (
                    <div className="mt-2 bg-purple-50 border border-primary/20 rounded-lg px-3 py-2 flex items-center gap-2">
                      <span className="text-xs font-bold text-primary">
                        🔄 New counter: PKR {app.lastCounterAmount?.toLocaleString()}
                      </span>
                    </div>
                  )}
                </div>
