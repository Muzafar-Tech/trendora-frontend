...
export default function BrandProfile() {
  ...
  const handlePasswordChange = async (e) => {
    e.preventDefault()
    if (passForm.newPassword !== passForm.confirmPassword) return showToast('Passwords do not match', 'error')
    if (passForm.newPassword.length < 6) return showToast('Minimum 6 characters', 'error')
    setPassLoading(true)
    try {
  ...
  return (
    <DashboardLayout links={brandLinks}>
      ...
      <div className="mb-6">
        ...
        <p className="text-muted text-sm mt-1">Manage your brand profile and account settings.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-card rounded-2xl border border-border shadow-card p-6 text-center">
            ...
            <h3 className="font-bold text-secondary text-lg">{user?.brandName || user?.fullName}</h3>
